...
function changeSize(numSlides){
          ...
          if (largura<=720){
            ...
            $('#nav').width(largura);
            $('.ctn-content').width(largura).height(altura+40);     
          }else{
...
$.fn.cycle.transitions.scrollVert = {
    before: function( opts, curr, next, fwd ) {
        ...
        opts.animIn = { top: 0 };
        if(fwd==true){
          opts.cssBefore = { top: false ? +height : height, left: 0, opacity: 1, display: 'block', visibility: 'visible' };
          opts.animOut = { top: false ? height : -height };
        }else{
          opts.cssBefore = { top: true ? -height : height, left: 0, opacity: 1, display: 'block', visibility: 'visible' };
